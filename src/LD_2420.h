...
inline uint32_t LD2420_BAUD = 115200UL; // Основная скорость UART для новых прошивок LD2420.
inline uint32_t LD2420_ALT_BAUD = 256000UL; // Альтернативная скорость UART для некоторых старых прошивок LD2420.
inline uint32_t LD2420_BAUD_RETRY_MS = 2500UL; // Время ожидания валидного кадра перед попыткой переключения на альтернативный baud.
...
inline String LD2420_ACTIVE_BAUD_TEXT = "115200"; // Текст текущей рабочей скорости UART.
...
static uint32_t gCurrentBaud = 115200UL; // Текущая активная скорость UART.
static bool gBaudLocked = false; // Флаг, что рабочая скорость уже найдена по валидному кадру.
static bool gTriedAltBaud = false; // Флаг, что попытка перехода на альтернативный baud уже была выполнена.
...
static inline void ld2420ApplyRuntimeUartConfigIfChanged() { // Отслеживаем изменение UART-параметров из UI и применяем их на лету.
  ...
  const bool baudChanged = (gAppliedBaud != LD2420_BAUD); // Проверяем, изменился ли основной baud.
  ...
  ld2420BeginSerial(LD2420_BAUD); // Перезапускаем UART с актуальными настройками.
  ...
  gBaudLocked = false; // Сбрасываем флаг фиксации рабочей скорости.
  gTriedAltBaud = false; // Разрешаем новую попытку fallback на альтернативный baud.
...
static inline void ld2420HandleUserCommands() { // Выполняем команды управления LD2420 из UI-флагов.
  ...
  if (LD2420_CMD_REBOOT) { ld2420SendReboot(); LD2420_CMD_REBOOT = false; gDriverStartedAt = millis(); LD2420_LAST_FRAME_AT_MS = 0; gBaudLocked = false; gTriedAltBaud = false; } // Перезагружаем датчик, сбрасываем флаг и перезапускаем watchdog восстановления потока.
  if (LD2420_CMD_FACTORY_RESET) { ld2420SendFactoryReset(); LD2420_CMD_FACTORY_RESET = false; gDriverStartedAt = millis(); LD2420_LAST_FRAME_AT_MS = 0; gBaudLocked = false; gTriedAltBaud = false; } // После factory reset повторно запускаем recovery-логику как после холодного старта.
...
static inline void ld2420TryAutoRecovery(uint32_t now) { // Автоматически восстанавливаем поток данных, если датчик после рестарта молчит.
  ...
  ld2420BeginSerial(LD2420_BAUD); // Перед recovery принудительно возвращаемся на основной baud для надежной команды.
...
static inline void setup_LD2420() { // Инициализируем драйвер LD2420 и стартуем UART.
  ...
  ld2420BeginSerial(LD2420_BAUD); // Запускаем UART на основном baud.
...
static inline void loop_LD2420() { // Главный цикл обслуживания датчика LD2420 (вызывается из loop()).
  ...
  if (!gBaudLocked && !gTriedAltBaud && LD2420_LAST_FRAME_AT_MS == 0 && now >= LD2420_BAUD_RETRY_MS) { // Если данных нет, пробуем альтернативный baud.
    ld2420BeginSerial(LD2420_ALT_BAUD); // Переключаем UART на альтернативную скорость.
    gTriedAltBaud = true; // Помечаем, что fallback уже выполнен.
    LD2420_LINK_STATUS = "Проверка ALT baud"; // Показываем в UI текущую диагностическую фазу.
  }
  ...
  if (reading.frameValid) { // Если пришел валидный кадр, обновляем все рабочие переменные.
    gBaudLocked = true; // Фиксируем рабочую скорость UART.
    ...
    if (reading.distanceM > 0.01f && reading.distanceM < 15.0f) { // Проверяем, что расстояние лежит в реалистичном диапазоне.
      gLastDistanceM = ld2420FilterDistance(reading.distanceM); // Пропускаем raw через фильтры, чтобы повысить стабильность и точность отображения.
    }
  }
