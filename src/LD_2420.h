...
inline uint8_t LD2420_UART_PORT = 1; // Номер аппаратного UART ESP32 (обычно 1 или 2 для внешнего датчика).
...
inline int LD2420_MAX_FRAMES_PER_LOOP = 6; // Сколько валидных кадров максимум обрабатываем за один проход loop для ускорения опроса.
...
static inline void loop_LD2420() { // Главный цикл обслуживания датчика LD2420 (вызывается из loop()).
  ...
  const int maxFrames = constrain(LD2420_MAX_FRAMES_PER_LOOP, 1, 20); // Ограничиваем верхний предел, чтобы не зациклить loop на шумном UART.
