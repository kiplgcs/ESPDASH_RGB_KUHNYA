...
constexpr int HLK_LD2410C_RX_PIN = 16; // RX-пин ESP32 для приема данных от TX HLK-LD2410C.
constexpr int HLK_LD2410C_TX_PIN = 17; // TX-пин ESP32 для передачи команд в RX HLK-LD2410C.
constexpr uint32_t HLK_LD2410C_BAUD = 256000; // Скорость UART для HLK-LD2410C (стандартная для модуля).
constexpr uint8_t HLK_LD2410C_UART_PORT = 2; // Используем аппаратный порт Serial2 на ESP32.

// -------------------- Настройки автоматики кухни --------------------
constexpr float KITCHEN_DISTANCE_NEAR_ENTER_M = 3.0f; // Порог входа в ближнюю зону (подход к столешнице).
constexpr float KITCHEN_DISTANCE_NEAR_EXIT_M = 3.4f; // Порог выхода из ближней зоны (гистерезис вверх).
constexpr float KITCHEN_DISTANCE_FAR_ENTER_M = 3.8f; // Порог входа в дальнюю зону (гистерезис вниз/вверх).
constexpr float KITCHEN_DISTANCE_FAR_EXIT_M = 3.2f; // Порог выхода из дальней зоны (возврат к рабочей зоне).

constexpr uint32_t KITCHEN_APPROACH_ANIMATION_MS = 1100UL; // Длительность анимации включения от краев к центру.
constexpr uint32_t KITCHEN_TRANSITION_WAIT_MS = 20000UL; // Задержка перед переходом из TRANSITION в AMBIENT.
constexpr uint32_t KITCHEN_AMBIENT_DURATION_MS = 600000UL; // Время работы декоративного режима (10 минут).
constexpr uint32_t KITCHEN_SENSOR_CONFIRM_MS = 700UL; // Время подтверждения смены зоны по датчику.
constexpr uint32_t KITCHEN_SIGNAL_HOLD_MS = 1500UL; // Время удержания последней валидной цели при кратком пропадании.
constexpr uint32_t KITCHEN_FADE_IN_MS = 400UL; // Время плавного включения/подъема яркости.
constexpr uint32_t KITCHEN_FADE_OUT_MS = 900UL; // Время плавного выключения/спада яркости.

constexpr uint8_t KITCHEN_WORK_WHITE_BRIGHTNESS = 220; // Яркость белого рабочего режима.
constexpr uint8_t KITCHEN_AMBIENT_BRIGHTNESS = 96; // Яркость декоративного режима.
constexpr uint16_t KITCHEN_FILTER_WINDOW = 6; // Окно скользящего среднего для сглаживания дистанции.
...
static HardwareSerial HLKSerial(HLK_LD2410C_UART_PORT); // Создаем UART-экземпляр для HLK-LD2410C.
...
static float gDistanceWindow[KITCHEN_FILTER_WINDOW] = {0}; // Буфер скользящего среднего по расстоянию.
...
static inline SensorReading readHlkSensor() { // Читаем сырые данные от HLK-LD2410C через UART.
    ...
    while (HLKSerial.available() > 0) { // Вычитываем все накопленные символы из UART-буфера.
        String line = HLKSerial.readStringUntil('\n'); // Берем строку до перевода строки.
...
static inline void updateDistanceFilter(const SensorReading &reading) { // Обновляем фильтрацию шумных измерений.
    ...
    gDistanceIndex = (gDistanceIndex + 1) % KITCHEN_FILTER_WINDOW; // Сдвигаем индекс записи по кольцу.
    if (gDistanceCount < KITCHEN_FILTER_WINDOW) { gDistanceCount++; } // Наращиваем заполненность буфера до его размера.
...
static inline void setup_HLK_LD2410C() { // Публичная инициализация датчика и автоматики.
    HLKSerial.begin(HLK_LD2410C_BAUD, SERIAL_8N1, HLK_LD2410C_RX_PIN, HLK_LD2410C_TX_PIN); // Поднимаем UART-связь с HLK-LD2410C.
